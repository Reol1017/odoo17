    ...
    def _process_ocr_result(self, result):
        ...
        if isinstance(words_result, list) and len(words_result) > 0:
            ...
            if 'result' in first_invoice:
                ...
                # 提取金额信息
                total_amount = self._extract_amount(invoice_data, 'TotalAmount')
                total_tax = self._extract_amount(invoice_data, 'TotalTax')
                amount_in_figures = self._extract_amount(invoice_data, 'AmountInFiguers')  # 注意：API字段拼写
                ...
                processed_data.update({
                    ...
                    'amount_in_figuers': amount_in_figures,
                })
